...
const ItemForm = ({ title, buttonName }) => {
  ...
  const deletePicture = async (id) => {
    await axios.delete(`/picture-service/${id}`, {
      headers: {
  ...
  const uploadFileToServer = async (id) => {
    ...
    await axios.post(`/picture-service/items/${id}`, formData, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('jwt')}`,
      },
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      let item;
      if (!id) {
        const response = await axios.post('/item-service/', data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('jwt')}`,
          },
        });
        ...
        const response = await axios.patch(`/item-service/${id}`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('jwt')}`,
          },
        });
