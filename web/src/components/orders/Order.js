...
const Order = () => {
  ...
  const cancelPayment = async (orderId) => {
    await axios.patch(`/payment-service/${orderId}`, { active: false });
  };

  const handleStripeToken = async (orderId, token) => {
    await axios.patch(`payment-service/${orderId}/stripe`, {
      token: token.id,
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`/payment-service/?page=${page}`);
    ...
    const hasNextPage = async () => {
      const response = await axios.get(`/payment-service/?page=${page + 1}`);
