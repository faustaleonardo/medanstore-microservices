...
const Checkout = () => {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      setLoading(true);
      const response = await axios.get('/shipping-service/cities');
  ...
  const handleVoucherSubmit = async (event) => {
    ...
    if (!voucher) return setVoucherError('Voucher code must not be empty!');

    try {
      setLoading(true);
      const response = await axios.get(`/voucher-service/code/${voucher}`);
      const result = response.data.data.data;

      if (voucherError) setVoucherError(null);
      setDiscount(result.discount);
    } catch (err) {
      setVoucherError(err.response.data);
    }
    setLoading(false);
    setVoucher('');
  };
  ...
  const handleFinishOder = async () => {
    ...
    try {
      ...
      await axios.post(`/payment-service/${orderId}`, data);
  ...
  return (
    <Fragment>
      ...
      <div className="mt-5">
        ...
        {renderWarningAlert(voucherError)}
        {discount ? (
